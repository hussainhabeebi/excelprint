...
export function CompanyCatalogueCard({ item }: { item: CompanyCatalogueItem }) {
  ...
  const card = (
    <Card className={`h-full overflow-hidden rounded-2xl border-border bg-white py-0 shadow-sm transition-[transform,border-color,box-shadow] duration-300 ease-out motion-reduce:transform-none motion-reduce:transition-none ${item.href ? "group-hover:-translate-y-1.5 group-hover:border-brand/70 group-hover:shadow-xl group-focus-visible:-translate-y-1.5 group-focus-visible:border-brand/70 group-focus-visible:shadow-xl" : ""}`}>
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-brand-soft/40">
        {image ? (
          <Image src={image.src} alt={image.alt} fill sizes="(min-width: 1280px) 25vw, (min-width: 1024px) 33vw, (min-width: 640px) 50vw, 100vw"
            className={`object-cover transition-transform duration-300 ease-out motion-reduce:transform-none motion-reduce:transition-none ${item.href ? "group-hover:scale-[1.035] group-focus-visible:scale-[1.035]" : ""}`} />
        ) : (
      ...
      <div className="flex min-h-44 flex-col p-5">
        ...
        <h3 className={`mt-1.5 text-lg font-semibold leading-snug text-foreground transition-colors duration-300 ${item.href ? "group-hover:text-brand group-focus-visible:text-brand" : ""}`}>{item.name}</h3>
